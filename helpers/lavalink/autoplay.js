...
async function fetchRelatedTrack(referenceTrack)
{
    ...
    try
    {
        if(sourceName === 'youtube' || identifier)
        {
            try
            {
                ...
                if(radioRes?.tracks?.length > 1)
                {
                    ...
                    {
                        Log.info(
                            "Autoplay found related track (YouTube Mix)",
                            "",
                            `source=${title}`,
                            `found=${track.info?.title}`
                        );
        ...
        const candidates = searchRes.tracks
            .filter(t => t?.info?.identifier !== identifier)
            .slice(0, 10);
        ...
        Log.info(
            "Autoplay found related track (Search)",
            "",
            `source=${title}`,
            `found=${selected.info?.title}`
        );
