...
- **`HeroField`** — the opening plate. A 96-bar waveform under a cosine envelope on a 1440x620 viewBox at `preserveAspectRatio="slice"`, crossed by a full-height playhead. The page ground is painted back over it in a vertical ramp rather than masked, so the copy above it only ever sits on a darker surface.
