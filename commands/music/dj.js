...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addRoleOption((option) =>
          option.setName("role").setDescription("Existing role to use (leave empty to auto-create)").setRequired(false)
        )
    ...
    .addSubcommand((sub) => sub.setName("disable").setDescription("Disable DJ mode"))
    .addSubcommand((sub) =>
      sub
        .setName("setrole")
        .setDescription("Update the DJ role")
        .addRoleOption((option) =>
          option.setName("role").setDescription("Role that will be considered DJ").setRequired(true)
        )
    )
    .addSubcommand((sub) =>
      sub
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addBooleanOption((option) =>
          option.setName("strict").setDescription("Require the DJ role to manage DJ mode").setRequired(true)
        )
    )
    .addSubcommand((sub) => sub.setName("status").setDescription("Show current DJ configuration")),
    ...
    try {
      ...
      if (!requireManagement(interaction, config)) {
        const noAccessMessage =
          config.strictMode && config.roleId
      ...
      if (subcommand === "enable") {
        ...
        if (!roleInfo.role) {
          await respond(interaction, {
            embeds: [errorEmbed("Unable to create or locate a DJ role. Make sure I can manage roles.")],
          });
          return;
        }
        ...
        const threshold = thresholdPercent
          ? Math.max(0.1, Math.min(1, thresholdPercent / 100))
          : djStore.DEFAULT_CONFIG.voteThreshold;
      ...
      const lines = [
        ...
        "Strict permissions: " +
          (config.strictMode
            ? config.roleId
              ? "DJ role only"
              : "DJ role only (role not set)"
            : "Server managers and DJ role"),
      ];
      ...
      await respond(interaction, {
        embeds: [errorEmbed("Could not update DJ settings.", "Please try again later.")],
      });
    }
  },
};
