        ...
        async execute(interaction)
        {
            ...
            if(config.enabled && !isDj)
            {
                try
                {
                    ...
                    return interaction.editReply({ embeds: [successEmbed('Your suggestion has been sent to the DJ for approval.')], allowedMentions: { parse: [] } });
                }
                catch(error)
                {
                    Log.error('Failed to prepare suggestion preview', error);
                    return interaction.editReply({ embeds: [errorEmbed('Could not prepare that track. Please try a different query.')] });
                }
            }
